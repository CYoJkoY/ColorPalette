...
const RELATIONS = [
  ['analogous', 'Analogous', 'Adjacent hues with a calm, cohesive feel.'],
  ['complementary', 'Complementary', 'Opposite hues for strong visual separation.'],
  ['split', 'Split complementary', 'One base hue with two near-opposite accents.'],
  ['triadic', 'Triadic', 'Three evenly spaced hues for balanced contrast.'],
  ['tetradic', 'Tetradic', 'Four hues arranged as two complementary pairs.'],
  ['doubleComplementary', 'Double complementary', 'Two neighboring hues paired with their opposites.'],
  ['warm', 'Warm range', 'A warm shift around the source hue.'],
  ['cool', 'Cool range', 'A cool shift around the source hue.'],
  ['monochromatic', 'Monochromatic', 'One hue across several lightness levels.'],
  ['tints', 'Tints', 'Lighter, softer versions of the source hue.'],
  ['shades', 'Shades', 'Darker versions of the source hue.'],
  ['tones', 'Tones', 'Lower-saturation versions for restrained interfaces.']
];
...
function swatches(hexes) {
  return hexes.map((hex, index) => ({ hex, primary:index === 0 }));
}
...
Page({
  ...
  loadColor(value) {
    ...
    const contrast = [
      { label:'On white', background:'#FFFFFF', ratio:color.contrastRatio(value, '#FFFFFF') },
      { label:'On black', background:'#000000', ratio:color.contrastRatio(value, '#000000') }
    ].map(x => ({ ...x, ratio:x.ratio.toFixed(2), grade:color.contrastGrade(Number(x.ratio)) }));
    this.setData({
      item,
      rgb,
      hsl,
      labText:`${lab.L.toFixed(4)} · ${lab.a.toFixed(4)} · ${lab.b.toFixed(4)}`,
      labCss:`oklab(${lab.L.toFixed(4)} ${lab.a.toFixed(4)} ${lab.b.toFixed(4)})`,
      lchText:`${lch.L.toFixed(4)} · ${lch.C.toFixed(4)} · ${lch.H.toFixed(2)}°`,
      lchCss:`oklch(${(lch.L * 100).toFixed(2)}% ${lch.C.toFixed(4)} ${lch.H.toFixed(2)})`,
      related,
      palette:advanced.makeScale(value),
      relations,
      contrast,
      favorite:getFavorites().some(x => x.id === item.id)
    });
  },
