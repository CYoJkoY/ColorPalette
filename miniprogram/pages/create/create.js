...
function parsePalette(value) {
  try {
    ...
    return { id:parsed.id || '', name:parsed.name || '我的色卡', colors:parsed.colors.slice(0, 8) };
  } catch (_) { return null; }
}
...
Page({
  data: {
    ...
    paletteId:'', paletteName:'我的色卡', editing:false,
  ...
  onLoad(options = {}) {
    ...
    if (palette) {
      const first = color.hexToRgb(palette.colors[0]);
      const hsl = first ? color.rgbToHsl(first.r, first.g, first.b) : { h:262, s:80, l:52 };
      this.setData({ paletteId:palette.id, paletteName:palette.name, editing:true, colors:palette.colors, hex:palette.colors[0] || '#7C3AED', ...hsl }, () => this.refreshLibrary());
      return;
    }
    ...
    if (options.colors) {
      try {
        const colors = JSON.parse(decodeURIComponent(options.colors)).filter(Boolean).slice(0, 8);
        if (colors.length) this.setData({ colors, hex:colors[0] }, () => this.refreshLibrary());
      } catch (_) { this.refresh(); this.refreshLibrary(); }
      return;
    }
    this.refresh();
    this.refreshLibrary();
  ...
  inputHex(e) {
    ...
    this.setData({ hex:color.rgbToHex(rgb.r, rgb.g, rgb.b), h:hsl.h, s:hsl.s, l:hsl.l }, () => { if (!this.data.editing) this.refresh(); });
  },
  ...
  onSliderChange(e) {
    ...
    this.setData({ ...next, hex:color.rgbToHex(rgb.r, rgb.g, rgb.b), editing:false }, () => this.refresh());
  },
  selectLibraryColor(e) {
    const hex = e.currentTarget.dataset.hex;
    const rgb = color.hexToRgb(hex);
    const hsl = color.rgbToHsl(rgb.r, rgb.g, rgb.b);
    this.setData({ hex, h:hsl.h, s:hsl.s, l:hsl.l, editing:false }, () => this.refresh());
  },
  addLibraryColor(e) {
    const colors = this.data.colors.slice();
    if (colors.length >= 8) return wx.showToast({ title:'最多 8 个颜色', icon:'none' });
    const hex = e.currentTarget.dataset.hex.toUpperCase();
    colors.push(hex);
    upsertColor(hex);
    this.setData({ colors, editing:true });
  },
  addColor() {
    const colors = this.data.colors.slice();
    if (colors.length >= 8) return wx.showToast({ title:'最多 8 个颜色', icon:'none' });
    colors.push(this.data.hex);
    upsertColor(this.data.hex);
    this.setData({ colors, editing:true });
  },
  removeColor(e) {
    ...
    if (colors.length < 1) return wx.showToast({ title:'至少保留 1 个颜色', icon:'none' });
    this.setData({ colors, editing:true });
  },
  moveColor(e) {
    const index = Number(e.currentTarget.dataset.index);
    ...
    if (target < 0 || target >= this.data.colors.length) return;
    const colors = this.data.colors.slice();
  ...
  save() {
    ...
    const now = Date.now();
    const existing = getWorkspace().palettes.find(x => x.id === this.data.paletteId);
    ...
    this.setData({ paletteId:item.id, paletteName:item.name, saved:true, editing:true });
    toggleFavorite({ id:item.id, name:item.name, colors:item.colors, createdAt:item.createdAt, source:item.source });
    wx.showToast({ title:'色卡已保存', icon:'none' });
  },

  exportPalette() {
    if (!this.data.colors.length) return;
    const formats = buildExport(this.data.colors, this.data.paletteName);
    const text = `CSS\n${formats.css}\n\nSCSS\n${formats.scss}\n\nJSON\n${formats.json}\n\nDesign Tokens\n${formats.tokens}\n\nTailwind\n${formats.tailwind}`;
    wx.setClipboardData({ data:text, success:() => wx.showToast({ title:'已复制全部导出格式', icon:'none' }) });
  }
});
